...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($paymentOption === 'new') {
        ...
        $insertMasterQuery = "INSERT INTO tbl_Booking_master (Customer_id, Bus_route_id, Farestage_id, Booking_date, Total_fare, Distance,Total_seats)
        VALUES ('$customerID', '$busRouteID', '$fareStageID', '$journeyDate', '$totalCost', '$distance','$passcount')";
       ...
       $insertChildQuery = "INSERT INTO tbl_Booking_child (Booking_master_id, Passenger_name, Age, Gender, Seat_number, Sstop_no, Dstop_no, Source, Destination, Booking_status)
       VALUES ('$booking_master_id', '$passengerName', '$passengerAge', '$passengerGender', '$passengerSeat','$Sstop_no','$Dstop_no', '$start', '$destination', 1)";
                ...
                if ($conn->query($cardInsertSql) === TRUE) {
                    ...
                    if ($conn->query($paymentInsertSql) === TRUE) {
                        ...
                        if ($conn->query($ticketInsertSql) === TRUE) {
                            ...
                            //echo '<div class="alert alert-success">Ticket purchased successfully...please wait for a while</div>';
                            echo '<script>
                            setTimeout(function() {
                            window.location.href = "pbuffer.php?&ticket_id=' . $Ticket_id . '";}); 
                        </script>';
                        exit();

                        } else {
                            echo 'Error inserting ticket details: ' . $conn->error;
                        }
                    } else {
                        echo 'Error inserting payment details: ' . $conn->error;
                    }
                } else {
                    echo 'Error adding new card: ' . $conn->error;
                }
            }
        }
       ...
       echo 'Error inserting booking master details: ' . mysqli_error($conn);
        ...
        $existingCardQuery = "SELECT Card_id, Cvv, Card_no FROM tbl_Card WHERE Card_id = '$existingCardID' AND Customer_id = '$customer_id'";
        $existingCardResult = $conn->query($existingCardQuery);
        ...
        if ($existingCardResult->num_rows > 0) {
            ...
            if ($enteredCvv == $storedCvv) {
                ...
                echo '<div class="alert alert-danger">CVV is incorrect.</div>';
            }
        }
    }
}
            ...
            <div class="credit-card">
                <div class="credit-card-front">
                    ...
                    <div class="date-cvv-container">
                        <input type="month" class="form-control exp-date" name="expDate" min="<?= date('Y-m'); ?>" >
