...
$sql = "SELECT db.Driver_bus_id, db.Allocation_date, db.Start_time, db.End_time, db.Db_status,
               CONCAT(d.Driver_id, '.', d.D_fname) AS DriverInfo,
               CONCAT(b.Bus_id, '.', b.Bus_Reg_no) AS BusInfo
        FROM tbl_Driver_Bus AS db
        INNER JOIN tbl_Driver AS d ON db.Driver_id = d.Driver_id
        INNER JOIN tbl_Bus AS b ON db.Bus_id = b.Bus_id
        WHERE db.Driver_bus_id LIKE '%$search%'
           OR b.Bus_id LIKE '%$search%'
...
<head>
    ...
    <link rel="stylesheet" href="Driverbus.css"> 
...
<!-- <button class="toggle-button" id="menu">|||</button> -->
                ...
                <div class="user">
<a href="Adriverbus.php" class="btn">Assign Driver</a>
<div class="img-case">
                ...
                <table id="table" class="content-table pdf">
                    ...
                    <tbody>
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<td>" . $row["BusInfo"] . "</td>";
                            echo "<td>" . $row["DriverInfo"] . "</td>";
                            echo "<td>" . $row["Allocation_date"] . "</td>";
                            echo "<td>" . $row["Start_time"] . "</td>";
                            echo "<td>" . $row["End_time"] . "</td>";
                            echo "<td>" . ($row["Db_status"] == 1 ? 'Active' : 'Inactive') . "</td>";
                            echo '<td class="dropdown">
                            <button class="dropbtn">Actions</button>
                            <div class="dropdown-content">
                                <a href="Udriverbus.php?Driver_bus_id=' . $row["Driver_bus_id"] . '">Edit</a>
                                <a href="Dbdeactivate.php?Driver_bus_id=' . $row["Driver_bus_id"] . '">Deactivate</a>
                            </div>
                        </td>';
                        echo "</tr>";
                            $serialNumber++;
