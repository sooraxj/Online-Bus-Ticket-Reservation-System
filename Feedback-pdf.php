...
$sql = "SELECT f.*, c.C_fname, t.Ticket_number 
                FROM tbl_Feedback f
...
while ($row = $result->fetch_assoc()) {
    $a .= '
        <tr>
            ...
            <td>' . $row['Bus_rating'] . '</td>
            <td>' . $row['Driver_rating'] . '</td>
            <td>' . $row['Comment'] . '</td>
            <td>' . ($row['Complaint'] == 0 ? 'No' : 'Yes') . '</td>
...
$mpdf->WriteHTML($a);
$mpdf->Output('Feedback-reports.pdf', 'F');
header('Content-Type: application/pdf');
header('Content-Disposition: attachment; filename="Feedback-reports.pdf"');
readfile('Feedback-reports.pdf');
?>
